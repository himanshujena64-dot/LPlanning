...
def load_bom_from_bytes(bom_bytes):
    """
    Load BOM from either:
    • SAP tab-delimited TXT export  (detected by non-ZIP magic bytes)
    • XLSX / XLS workbook

    TXT column mapping:
      Explosion Level → Level
      FG_Com          → BOM Header
      Parent_Com      → Parent  (already present — no need to rebuild)
      Material Description → Component descriptio
      Proc_type       → Procurement type
      Required Quantity → Required Qty
      Sp. Procurement Type → Special procurement
    """
    is_xlsx = bom_bytes[:2] == b'PK'  # ZIP / XLSX magic
    ...
    if is_xlsx:
        ...
        # Build Parent column if not present
        if "Parent" not in bom.columns:
            parents, stack = [], {}
        ...
        # SAP TXT tab-delimited export
        text = bom_bytes.decode("latin-1")
        lines = text.splitlines()
        # Find header row (contains "Explosion Level" or "FG_Com")
        header_idx = None
        for i, line in enumerate(lines):
            if "Explosion Level" in line or ("FG_Com" in line and "Component" in line):
                header_idx = i
                break
        if header_idx is None:
            raise ValueError("Could not locate header row in BOM TXT file.")
        content = "\n".join(lines[header_idx:])
        bom = pd.read_csv(io.StringIO(content), sep="\t", dtype=str, keep_default_na=False)
        bom.columns = [str(c).strip() for c in bom.columns]
        # Remove completely empty rows
        bom = bom[bom.apply(lambda r: any(str(v).strip() for v in r), axis=1)].copy()
        # Column rename map
        col_map = {
            "Explosion Level": "Level",
            "FG_Com":          "BOM Header",
        ...
        if "Alt" not in bom.columns:
            bom["Alt"] = "0"
        if "BOM header descripti" not in bom.columns:
            bom["BOM header descripti"] = ""
        bom["Level"] = pd.to_numeric(bom.get("Level", pd.Series(["0"] * len(bom))), errors="coerce").fillna(0).astype(int)
        bom = bom.reset_index(drop=True)
        # Parent_Com is already present in TXT; if still missing, rebuild it
        if "Parent" not in bom.columns:
            parents, stack = [], {}
            for i in range(len(bom)):
                lvl = bom.loc[i, "Level"]
    ...
    for col, default in [("Alt", "0"), ("Special procurement", ""), ("Procurement type", ""),
                         ("Component descriptio", ""), ("BOM header descripti", "")]:
    ...
    bom["Required Qty"]        = pd.to_numeric(
        bom["Required Qty"].astype(str).str.replace(",", "", regex=False).str.strip(),
        errors="coerce").fillna(0)
    bom["Alt"] = pd.to_numeric(bom["Alt"], errors="coerce").fillna(0).astype(int).astype(str)
    # Remove level-0 (FG header rows) from explosion
    bom = bom[bom["Level"] > 0].copy().reset_index(drop=True)
    return bom


def load_mb52_stock(mb52_bytes):
    """
    Load stock from SAP MB52 tab-delimited TXT export.
    Sums  Unrestricted + Quality Inspection  per Material (group-by).
    Returns pd.Series indexed by Material code.
    """
    try:
        text = mb52_bytes.decode("latin-1")
        lines = text.splitlines()
        # Find header row containing 'Material' and 'Unrestricted'
        header_idx = None
        for i, line in enumerate(lines):
            if "Material" in line and "Unrestricted" in line:
                header_idx = i
                break
        if header_idx is None:
            raise ValueError("Cannot find header row in MB52 TXT file.")
        content = "\n".join(lines[header_idx:])
        df = pd.read_csv(io.StringIO(content), sep="\t", dtype=str, keep_default_na=False)
        df.columns = [str(c).strip() for c in df.columns]
        # Identify columns
        mat_col = next((c for c in df.columns if c.lower() == "material"), df.columns[0])
        unr_col = next((c for c in df.columns if "unrestricted" in c.lower() and "value" not in c.lower()), None)
        qi_col  = next((c for c in df.columns if "quality" in c.lower() and "value" not in c.lower()), None)
        df[mat_col] = df[mat_col].astype(str).str.strip()
        # Keep only rows where material looks like a part number (non-empty, not totals line)
        df = df[df[mat_col].str.strip().astype(bool)].copy()
        df = df[~df[mat_col].str.lower().str.startswith(("total", "grand", "sum"))].copy()
        ...
        df["__qty__"] = _to_num(df[unr_col]) if unr_col else 0
        if qi_col:
...
def load_prod_plan_xlsx(prod_bytes):
    """
    Load new Production Plan XLSX format.
    Columns: Type | FG_Com | <date1> | <date2> | ...
    Sums all date columns per FG_Com → returns DataFrame with
    columns [Component, Plan_Qty] (mapped to Confirmed_Qty for pivot).
    """
    ...
    try:
        ...
        df.columns = [str(c).strip() for c in df.columns]
        # Find FG_Com / component column
        fg_col = next((c for c in df.columns if "fg" in c.lower() or "component" in c.lower()
                       or "material" in c.lower()), df.columns[1] if len(df.columns) > 1 else df.columns[0])
        # Date columns = everything else except Type / FG_Com
        skip_cols = {c for c in df.columns if c.lower() in ("type",) or c == fg_col}
        date_cols = [c for c in df.columns if c not in skip_cols]
        df[fg_col] = df[fg_col].astype(str).str.strip()
        for col in date_cols:
            df[col] = pd.to_numeric(df[col], errors="coerce").fillna(0)
        df["Plan_Qty"] = df[date_cols].sum(axis=1)
        summary = (df.groupby(fg_col, as_index=False)["Plan_Qty"]
                   .sum()
                   .rename(columns={fg_col: "Component", "Plan_Qty": "Confirmed_Qty"}))
        summary["Open_Production_Qty"] = 0
        return summary[["Component", "Confirmed_Qty", "Open_Production_Qty"]]
    except Exception as e:
        st.warning(f"Production plan load warning: {e}")
        return empty_prod()
...
# ═══════════════════════════════════════════════════════════════
    ...
    def _file_status(key, label, hint="Accepted: .XLSX, .XLS"):
        ...
        if st.session_state.get(key):
            ...
            st.markdown(
                f'<p style="color:#9ca3af;font-size:11px;margin:2px 0 6px;">{hint}</p>',
                unsafe_allow_html=True)
    ...
    with c1:
        ...
        <p class="ucard-desc">SAP BOM export — TXT (tab-delimited) or XLSX</p></div></div>
        <span class="badge-req">Required</span></div></div>""", unsafe_allow_html=True)
        bf = st.file_uploader("BOM", type=["txt","xlsx","xls"], key="bom_u", label_visibility="collapsed")
        if bf: st.session_state["_bom"] = bf.read()
        _file_status("_bom", "BOM file", "Accepted: .TXT (SAP export), .XLSX, .XLS")
...
# ═══════════════════════════════════════════════════════════════
    ...
    topbar("Production Plan", "FG-wise daily production plan — aggregated by month")

    prod_plan_bytes = st.session_state.get("_prod_plan")
    bom_bytes       = st.session_state.get("_bom")

    # ── Upload if not yet loaded ──────────────────────────────
    sec("File Input")
    pf_up = st.file_uploader(
        "Production Plan XLSX (Type | FG_Com | date columns)",
        type=["xlsx","xls"], key="plan_up")
    if pf_up:
        prod_plan_bytes = pf_up.read()
        st.session_state["_prod_plan"] = prod_plan_bytes

    if not prod_plan_bytes:
        st.markdown("""<div class="empty"><div class="empty-icon">📋</div>
        <div class="empty-ttl">No production plan loaded</div>
        <div class="empty-sub">Upload the Production Plan XLSX file above.<br>
        Expected format: Type | FG_Com | 2026-05-26 | 2026-05-27 | …</div></div>""",
        unsafe_allow_html=True)
    else:
        import io as _io
        df_raw = pd.read_excel(_io.BytesIO(prod_plan_bytes))
        df_raw.columns = [str(c).strip() for c in df_raw.columns]

        # Identify columns
        type_col = next((c for c in df_raw.columns if c.lower() == "type"), None)
        fg_col   = next((c for c in df_raw.columns
                         if "fg" in c.lower() or "component" in c.lower()
                         or "material" in c.lower()),
                        df_raw.columns[1] if len(df_raw.columns) > 1 else df_raw.columns[0])

        skip_set = {c for c in [type_col, fg_col] if c}
        date_cols_raw = [c for c in df_raw.columns if c not in skip_set]

        # Parse date columns and aggregate by month
        month_map = {}   # month_label → [raw_col, ...]
        for col in date_cols_raw:
            try:
                ts = pd.Timestamp(col)
                if pd.notna(ts) and ts.year > 2000:
                    lbl = ts.strftime("%b-%y")
                    month_map.setdefault(lbl, []).append(col)
            except Exception:
                pass

        months_lbl = sorted(month_map.keys(),
                            key=lambda m: pd.Timestamp(f"01-{m}") if m != "" else pd.Timestamp("2000-01-01"))

        # Build month-aggregated dataframe
        df_plan = df_raw[[c for c in [type_col, fg_col] if c]].copy()
        if type_col: df_plan[type_col] = df_plan[type_col].astype(str).str.strip()
        df_plan[fg_col] = df_plan[fg_col].astype(str).str.strip()

        for lbl, cols in month_map.items():
            df_plan[lbl] = sum(
                pd.to_numeric(df_raw[c], errors="coerce").fillna(0) for c in cols)

        # Group by FG_Com (+ Type if present)
        grp_cols = ([type_col] if type_col else []) + [fg_col]
        df_plan = df_plan.groupby(grp_cols, as_index=False)[months_lbl].sum()

        # ── Get model descriptions from BOM ───────────────────
        if bom_bytes:
            try:
                bom_df = load_bom_from_bytes(bom_bytes)
                if "BOM header descripti" in bom_df.columns:
                    desc_lookup = (bom_df[["BOM Header","BOM header descripti"]]
                                   .drop_duplicates("BOM Header")
                                   .rename(columns={"BOM Header": fg_col,
                                                    "BOM header descripti": "Model Description"}))
                    df_plan = df_plan.merge(desc_lookup, on=fg_col, how="left")
                    df_plan["Model Description"] = df_plan["Model Description"].fillna("")
                else:
                    df_plan["Model Description"] = ""
            except Exception:
                df_plan["Model Description"] = ""
        else:
            df_plan["Model Description"] = ""
        ...
        with fc1:
            all_fgs = sorted(df_plan[fg_col].unique().tolist())
            sel_fgs = st.multiselect("Filter by FG Code", all_fgs,
                                     placeholder="All FG codes", key="plan_fgs")
        with fc2:
            if type_col:
                all_types = sorted(df_plan[type_col].unique().tolist())
                sel_types = st.multiselect("Filter by Type", all_types,
                                           placeholder="All types", key="plan_types")
            else:
                sel_types = []
        with fc3:
            search_desc = st.text_input("Search description", placeholder="e.g. ODU 1.5T",
                                        key="plan_search")

        plan_df = df_plan.copy()
        if sel_fgs:   plan_df = plan_df[plan_df[fg_col].isin(sel_fgs)]
        if sel_types and type_col: plan_df = plan_df[plan_df[type_col].isin(sel_types)]
        if search_desc:
            plan_df = plan_df[plan_df["Model Description"].str.contains(
                search_desc, case=False, na=False)]

        # ── Summary metrics ────────────────────────────────────
        sec("Plan Summary")
        n_metrics = min(len(months_lbl), 6)
        mc = st.columns(n_metrics + 1)
        mc[0].metric("Total FG Codes", f"{len(plan_df):,}")
        for i, m in enumerate(months_lbl[:n_metrics]):
            mc[i+1].metric(m, f"{int(plan_df[m].sum()):,}")

        # ── Main table ─────────────────────────────────────────
        sec("FG-wise Monthly Production Plan")
        plan_df["Total"] = plan_df[months_lbl].sum(axis=1)
        plan_df = plan_df.sort_values("Total", ascending=False).reset_index(drop=True)

        show_zero = st.checkbox("Include FGs with zero total", value=False, key="plan_zero")
        if not show_zero:
            plan_df = plan_df[plan_df["Total"] > 0]
        st.caption(f"{len(plan_df):,} FG codes shown")

        disp_cols = ([type_col] if type_col else []) + [fg_col, "Model Description"] + months_lbl + ["Total"]
        avail_cols = [c for c in disp_cols if c in plan_df.columns]
        fmt = {m: "{:,.0f}" for m in months_lbl + ["Total"]}

        def _plan_row_style(row):
            t = row.get("Total", 0)
            if t > 5000:  return ["background-color:#fff0f0"] * len(row)
            if t > 1000:  return ["background-color:#fffbeb"] * len(row)
            if t > 0:     return ["background-color:#f0fdf4"] * len(row)
            return [""] * len(row)
        ...
        # ── Bar chart ──────────────────────────────────────────
        sec("Total Planned Qty by Month")
        chart_data = pd.DataFrame({
            "Month": months_lbl,
            "Planned Qty": [int(plan_df[m].sum()) for m in months_lbl]
        })
        st.bar_chart(chart_data.set_index("Month"), use_container_width=True, height=280)

        # ── Type breakdown (if Type column exists) ────────────
        if type_col and not plan_df.empty:
            sec("Type-wise Breakdown")
            type_summ = (plan_df.groupby(type_col, as_index=False)[months_lbl + ["Total"]].sum()
                         .sort_values("Total", ascending=False))
            st.dataframe(type_summ.style.format(fmt), use_container_width=True, hide_index=True)
        ...
        with pd.ExcelWriter(buf, engine="openpyxl") as w:
            ...
            if type_col:
                type_summ.to_excel(w, sheet_name="Type Summary", index=False)
        buf.seek(0)
        st.download_button(
            "⬇ Download Production Plan (.xlsx)", data=buf,
            file_name="production_plan_monthly.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            use_container_width=True, type="primary")
